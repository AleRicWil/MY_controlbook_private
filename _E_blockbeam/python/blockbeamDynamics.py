...
class blockbeamDynamics:
    ...
    def f(self, state, u):
        ...
        #LHS
        M = np.array([[1, 0], 
                      [0, self.m1*z**2 + self.m2*self.length**2 / 3]])
        #RHS
        a = 2*self.m1*zdot*z*thetadot
        b = (self.m1*z + self.m2*self.length / 2)*self.g*np.cos(theta)
        c = F*self.length*np.cos(theta)
        C = np.array([[z*thetadot**2 - self.g*np.sin(theta)],
                      [-a-b+c]])
    ...
    def h(self):
        ...
        y = np.array([[z],[theta]])#,[zdot],[thetadot]])
        return y
